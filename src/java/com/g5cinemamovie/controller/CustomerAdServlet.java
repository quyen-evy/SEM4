...
public class CustomerAdServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            ArrayList<Customer> list = cusDAO.FindAll();
            int i = 1;
            for (Customer item : list) {
                out.print("<tr>"
                        + "<th scope='row'>" + item.getCusId() + "</th>"
                        ...
                        + "<td>" + item.getPhone() + "</td>"
                        + "<td>" + item.getEmail() + "</td>"
                        + "<td><a href=updatecustomer.jsp?id=" + item.getCusId() + " class=\"btn-success btn\">Update</a></td>"
                        + "<td><a class='btn-default btn' href=\\G5CinemaMovie\\DeleteCusServlet?id=" + item.getCusId() + ">Delete</a></td>"
                        + "</tr>");
                i++;
            }
            out.print("</tbody>");
